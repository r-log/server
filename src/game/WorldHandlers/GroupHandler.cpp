 ...
 *
 * Bit and byte order are transcribed from the client's own reader
 * (sub_14035A930); the two GUIDs interleave, so this cannot use the
 * single-GUID WriteGuidMask helpers.
 */
 ...
 *
 * Wire shape taken from the client's writer (sub_140315950): uint32 role, then
 * the affected unit as a packed GUID.
 *
 * @param recv_data The received opcode packet.
 */
...
void WorldSession::BuildPartyMemberStatsChangedPacket(Player* player, WorldPacket* data)
{
    ...
    *data << uint32(mask);
    ...
    if (mask & GROUP_UPDATE_FLAG_VEHICLE_SEAT)
    {
        ...
        {
            *data << uint32(((Unit*)player->GetTransportInfo()->GetTransport())->GetVehicleInfo()->GetVehicleEntry()->SeatID[player->GetTransportInfo()->GetTransportSeat()]);
        }
        else
        {
            *data << uint32(0);

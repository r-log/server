...
namespace
{
    // Chase distances, as fractions of the target's combat reach. The gap between the two
    // is the hysteresis band: we close to CHASE_RANGE, but only start closing again once
    // the target has drifted past CHASE_RECHASE_RANGE.
                                                    ...
                                                    Motion::Vector3 const& spot) const
{
    ...
    // `spot` is a FRAME point, so the target has to be read in the same frame. The
    // distance itself needs no correction once both ends are in one frame.
    Motion::IMotionFrame const& frame = Motion::FrameFor(owner);
    ...
    // IsWithinDist2d/3d fold it in, so this stays the same test it always was.
                                                     ...
                                                     uint32 diff)
{
    ...
    // the stale goal fails this test, and we derive a fresh spot and try again on the next
    // recheck -- instead of standing there believing we are already on our way.
    ...
    // A pet heeling its master is allowed to cheat its way to the exact spot; otherwise it
    // strands itself on the wrong side of scenery its master walked straight through.

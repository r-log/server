 ...
 * what happens on arrival. Everything that used to make these the most duplicated code in
 * the movement tree (owning a PathFinder, deciding when a moving goal is stale enough to
 * re-route, building a spline, sending the packet) now lives once, in the MotionDriver.
 *
 * What is left is the genuinely per-kind policy, expressed as the hooks below.
 */
                                  ...
                                  public TargetedMovementGeneratorBase
{
    ...
    protected:
        ...
        /// how far the target may drift before that spot is stale (true). The gap between
        /// the two is the hysteresis that stops a victim shuffling a yard inside melee
        /// from provoking a string of sub-yard catch-up legs.
        virtual float TargetDistance(Unit& owner, bool forRangeCheck) const = 0;

        /// The target is gone in a way only this kind can detect.
        ...
        /// often as a chaser, because a pet lagging behind its master reads far worse than
        /// a mob lagging a step behind its victim.

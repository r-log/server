...
// The opcode numbers are wire format: they are what the 4.3.4 client puts on the
// socket, so they belong to the protocol and not to the game. The dispatch table
// that binds an opcode to a WorldSession member function is the game's business
// and lives in game/Server/OpcodeTable.h.
//
// Splitting the two also retires a long-standing landmine. This header used to
// include WorldSession.h purely so that sizeof(OpcodeHandler) -- which embeds a
// pointer-to-member -- came out the same in every translation unit; under MSVC
// x64 release an incomplete WorldSession silently changed that size and the table
// was read with the wrong stride. With no pointer-to-member left in this file,
// there is nothing to get wrong.
//
// The other half of the old arrangement is gone with it: because the enum sat
// BELOW that include, any header reached through Opcodes.h could not name an
// opcode constant.
...
enum OpcodesList
{
    ...
    CMSG_SET_ROLE                                         = 0x25B1, // 4.3.4 15595 (capture-verified)
